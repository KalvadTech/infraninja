...
from infraninja.utils.pubkey_delete import (
    SSHKeyDeleter,
    SSHKeyDeleteError,
...
class TestSSHKeyDeleter(unittest.TestCase):
    ...
    @patch('infraninja.utils.pubkey_delete.Jinn')
    ...
    def test_fetch_ssh_keys_force_refresh(self, mock_login, mock_auth_request):
        ...
        # Set cached keys
        SSHKeyDeleter._ssh_keys = [{"id": "1", "label": "cached", "key": "ssh-rsa CACHED..."}]
    ...
    @patch('infraninja.utils.pubkey_delete.host')
    ...
    @patch('infraninja.utils.pubkey_delete.deploy')
    @patch.object(SSHKeyDeleter, '_check_root_access', return_value=False)
    def test_delete_ssh_keys_for_users_no_root_access(self, mock_root_check, mock_deploy):
        ...
        def mock_delete_func(users, filter_criteria=None, force_refresh=False):
            ...
            if not deleter._check_root_access():
                raise SSHKeyDeleteError("Root access required to modify other users' authorized_keys files.")
            return True
    ...
    def test_delete_ssh_keys_for_users_success(self, mock_root_check, mock_fetch, 
                                             mock_filter, mock_remove, mock_host):
        ...
        # Mock data
        mock_fetch.return_value = [{"id": "1", "label": "test", "key": "ssh-rsa TEST..."}]
...
class TestGlobalFunctions(unittest.TestCase):
    ...
    def test_delete_ssh_keys_for_users_global_function(self, mock_method):
        ...
        self.assertTrue(result)
        mock_method.assert_called_once_with(["user1", "user2"], {"labels": ["old"]}, False)
